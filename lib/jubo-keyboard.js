...
ddp.connect(function(error,wasReconnect) {
  ...
  var keyboard = {
    ...
    properties: [
      {service: 'switch',property: 'status',value: 'off',label:'运行状态'},
      {service: 'typer',property: 'zone-1',value: '1',label:'zone-1'},
      {service: 'typer',property: 'zone-2',value: '2',label:'zone-2'},
      {service: 'typer',property: 'zone-3',value: '3',label:'zone-3'},
      {service: 'typer',property: 'zone-4',value: '4',label:'zone-4'},
      {service: 'typer',property: 'zone-5',value: '5',label:'zone-5'},
      {service: 'typer',property: 'zone-6',value: '6',label:'zone-6'},
      {service: 'typer',property: 'zone-7',value: '7',label:'zone-7'}
    ]
  };
  ...
  ddp.call('add',[keyboard],
    ...
    Cylon.robot({
      ...
      work: function(my) {
        console.log('Key space start, backspace stop');
        my.keyboard.on('space',function(key) {
          zone.service = 'switch';
          zone.property = 'status';
          zone.value = 'on';

          ddp.call('adjust',[zone],function(error,pid) {
            console.log('space pressed, the model start');
          });
        });
        ...
        my.keyboard.on('backspace',function(key) {
          ...
          ddp.call('adjust',[zone],function(error,pid) {
            console.log('backspace pressed, the model stop');
          });
        });
